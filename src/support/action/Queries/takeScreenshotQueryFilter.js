 ...
 */
export default async(numberOfReceipts, searchCriteria, filterType, filterName) => {
    /**
    ...
    for (rowNumber= 0; rowNumber < parseInt(numberOfReceipts); rowNumber++) {
        if (parseInt(numberOfResults) >= rowNumber) {
            keyName = searchCriteria + '-' + filterName + '-' + filterType + '-RECEIPT-' + Number(parseInt(rowNumber)+1);
            await browser.pause(mediumPause);
            ...
            await browser.saveFullPageScreen(screenshotsFolder + "/" + keyName, { /* some options*/ });
            await browser.pause(mediumPause);
