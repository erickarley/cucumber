...
export default async(dimension, dataElement, measureGroup, measureToSelect) => {
     ...
     * @type {String}
     */
    const dimensionSelector = "//agilence-metric-selection[@selected-mapping='replaceController.selectedMappingPage2']//*[@ng-model='metricSelectionController.selectedMappingId']";

    /**
     * Selector for the dimension option
     * @type {String}
     */
    const dimensionOptionSelector = "//div[@class[contains(.,'md-active')]]//md-option[@ng-repeat='option in metricSelectionController.mappingOptions'][.='" + dimension + "']";

    /**Selector for the data Element
     * @type {String}
     */
    const dataElementSelector = "//agilence-metric-selection[@selected-mapping='replaceController.selectedMappingPage2']//*[@ng-model='metricSelectionController.selectedDataElementType']";

    /**
     * Selector for the data element option
     * @type {String}
     */
    const dataElementOptionSelector = "//div[@class[contains(.,'md-active')]]//*[@ng-selected='option.value == metricSelectionController.selectedDataElementType'][.='" + dataElement + "']";

   /**
     * Selector for the filter
     * @type {String}
     */
    const filterSelector = "//div[@ng-if='replaceController.page2']//input";

    /**
     * Selector for the Filter to Select
     * @type {String}
     */
    const measureFilterSelector = "//agilence-metric-selection[@selected-mapping='replaceController.selectedMappingPage2']//md-list-item[contains(.,'" + measureGroup + "')][1]";

    /**
     * Selector for the Measure to select
     * @type {String}
     */
    const measureSelector = "//agilence-metric-selection[@selected-mapping='replaceController.selectedMappingPage2']//md-list-item[contains(.,'" + measureToSelect + "')][1]";
