 ...
 * @param  {String}   aKPI              KPI selection
 */
export default async(kpiName, kpiType, dimension, aKPI) => {
    ...
    /**
      ...
      * @type {String}
      */
    const dataElementOptionSelector = "//md-option[@ng-repeat='option in metricSelectionController.elementTypes'][.='KPIs - Master']";
    ...
    await pause(1000);
    await select2020DropDown(dataElementSelector,dataElementOptionSelector,'KPIs - Master');
    await pause(1000);
    await setFilters(filterSelector,firstItemOnFiltersList,aKPI);
    await pause(1000);
    await clickElement('doubleClick','element',"//div[@id='itemListWithSearch']/md-list/md-list-item[1]/div[1]/button");
    await pause(1000);
}
